IRC is used by Sperocoin Electrum server to find 'peers' - other Sperocoin Electrum servers. The
current list can be seen by running:

    ./server.py peers

The following config file options are used by the IRC part of Sperocoin Electrum server:
...
Sperocoin Electrum server will run in private mode (default). In private mode, 
`./server.py peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Sperocoin Electrum server. It is
used both when binding the listener for incoming client connections and as part
of the real name field in IRC (see below).

`report_host` is a an optional fully-qualified domain name (FQDN) of your Sperocoin Electrum server 
...
composing the IRC nickname to identify your server on #sperocoin-electrum.
