...
current list can be seen by running:
    ...
    electrum-sperocoin-server peers
...
`electrum-sperocoin-server peers` will always return an empty list.
...
composing the IRC nickname to identify your server on #electrum-sperocoin.
...
iptables -t nat -A PREROUTING -p tcp --dport 110 -j REDIRECT --to-ports 5002
