...
The most up-to date version of this document is available at:
    ...
    https://github.com/DigitalCoin1/Electrum_Server/blob/master/HOWTO.md
...
### Step 1. Create a user for running sperocoind and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `sperocoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser sperocoin --disabled-password
    $ sudo apt-get install git
    $ sudo su - sperocoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/sperocoin/bin` in the output, you should add this line
...
Older versions of Electrum used to require a patched version of sperocoind.
This is not the case anymore since sperocoind supports the 'txindex' option.
We currently recommend sperocoind 2.6.4.9 stable.

If your package manager does not supply a recent sperocoind or you prefer to compile it yourself,
here are some pointers for Ubuntu:
    ...
    $ strip ~/src/SperoCoin/src/SperoCoind
    $ cp -a ~/src/SperoCoin/src/SperoCoind ~/bin/sperocoind

### Step 3. Configure and start sperocoind

In order to allow Electrum to "talk" to `sperocoind`, we need to set up an RPC
...
If you have a fresh copy of sperocoind start `sperocoind`:
...
irc.freenode.net #sperocoin-electrum and we'll try to congratulate you
