...
class BlockchainProcessor(Processor):
    ...
    def __init__(self, config, shared):
        ...
        self.sperocoind_url = 'http://%s:%s@%s:%s/' % (
            config.get('sperocoind', 'user'),
            config.get('sperocoind', 'password'),
            config.get('sperocoind', 'host'),
            config.get('sperocoind', 'port'))
    ...
    def sperocoind(self, method, params=[]):
        ...
        while True:
            try:
                respdata = urllib.urlopen(self.sperocoind_url, postdata).read()
                break
            except:
    ...
    def getfullblock(self, block_hash):
        ...
        try:
            ...
            logger.error("sperocoind error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run sperocoind with txindex=1; use -reindex if needed.")
